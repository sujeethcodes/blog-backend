...
controller.createPost = async (req, res) => {
    try {
        ...
        return res.json({ status: 200, message: "Post created", filePath: post.path });
    } catch (error) {
        console.error(error);
...
controller.editPost = async(req, res)=>{
    try {
        ...
        const updatePost = await Post.upload({ userId: findAccount?._id, _id : req?.body?.podtId}, {$set:{post: post.filename }});
        if (!updatePost) return res.status(406).json({ status: 406, message: "Post creation failed" });

        return res.json({ status: 200, message: "Post updated", filePath: post.path });
